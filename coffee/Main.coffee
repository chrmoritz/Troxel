...
$('#exportBase64').click ->
  ...
  if $('#exportBase64Tsl').prop('checked')
    $.post('https://trovesaurus.com/shorturl.php', {TroxelData: l}).done (url) -> $('#exportBase64Ta').val(url)
$('#exportJson').click ->
  return if io.readonly
...
$('#TroveCreationsExport').click ->
  ...
  if $('#TroveCreationsTsl').prop('checked')
    $.post('https://trovesaurus.com/shorturl.php', {TroxelData: l}).done (url) -> $('#TroveCreationsExportLink').val(url)
