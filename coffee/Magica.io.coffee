    ...
    fr.onloadend = =>
      ...
      if paletteBegin == -1 or paletteLength == -1
        ...
        for i in [0...paletteLength] by 4
          palette.push {r: rawpalette[i], g: rawpalette[i + 1], b: rawpalette[i + 2], a: rawpalette[i + 3], t: 0, s: 0}
        console.log "palette:"
        console.log palette
      # read voxel chunk
      @voxels = []
    ...
    for z in [0...@z] by 1
      for y in [0...@y] by 1
        for x in [0...@x] by 1 when @voxels[z]?[y]?[x]?
          rgba = ((@voxels[z][y][x].r << 24) | (@voxels[z][y][x].g << 16) | (@voxels[z][y][x].b << 8) | @voxels[z][y][x].a) >>> 0
          i = helpPalette[rgba]
          unless i?
            throw new Error "To many colors for Magica Voxel palette" unless paletteChunk.length < 1036
            paletteChunk = paletteChunk.concat [@voxels[z][y][x].r, @voxels[z][y][x].g, @voxels[z][y][x].b, @voxels[z][y][x].a]
